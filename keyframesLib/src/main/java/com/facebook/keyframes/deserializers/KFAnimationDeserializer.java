...
public class KFAnimationDeserializer {
    static final AbstractListDeserializer<KFAnimation> LIST_DESERIALIZER = new AbstractListDeserializer<KFAnimation>() {
        @Override
        ...
        while (reader.hasNext()) {
            ...
            switch (name) {
                case KFAnimation.PROPERTY_TYPE_JSON_FIELD:
                    builder.propertyType = KFAnimation.PropertyType.valueOf(reader.nextString().toUpperCase(Locale.US));
                    break;
                case KFAnimation.ANIMATION_FRAMES_JSON_FIELD:
                    builder.animationFrames = KFAnimationFrameDeserializer.LIST_DESERIALIZER.readList(reader);
                    break;
