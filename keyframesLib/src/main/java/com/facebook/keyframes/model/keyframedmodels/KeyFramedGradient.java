 ...
 * KFGradientColor.
 */
public class KeyFramedGradient extends KeyFramedObject<KFColorFrame, KeyFramedGradient.GradientColorPair> {
    /**
     ...
     */
    public static KeyFramedGradient fromGradient(KFGradientColor gradientColor, Position position) {
        return new KeyFramedGradient(gradientColor.getKeyValues(), gradientColor.getTimingCurves(), position);
    }

    private final Position mPosition;

    private KeyFramedGradient(List<KFColorFrame> objects, float[][][] timingCurves, Position position) {
     ...
     */
    @Override
    protected void applyImpl(KFColorFrame stateA, KFColorFrame stateB, float interpolationValue, GradientColorPair modifiable) {
        if (stateB == null) {
        ...
        if (mPosition == Position.START) {
            modifiable.setStartColor(getTransitionColor(interpolationValue, stateA.getColor(), stateB.getColor()));
        } else {
            modifiable.setEndColor(getTransitionColor(interpolationValue, stateA.getColor(), stateB.getColor()));
        }
    }
