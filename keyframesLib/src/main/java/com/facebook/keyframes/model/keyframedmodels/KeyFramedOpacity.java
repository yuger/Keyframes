 ...
 * KFAnimation.
 */
public class KeyFramedOpacity extends KeyFramedObject<KFAnimationFrame, KeyFramedOpacity.Opacity> {
    /**
    ...
    public KeyFramedOpacity(List<KFAnimationFrame> objects, float[][][] timingCurves) {
     ...
     */
    @Override
    protected void applyImpl(KFAnimationFrame stateA, KFAnimationFrame stateB, float interpolationValue, Opacity modifiable) {
        if (stateB == null) {
            modifiable.setOpacity(stateA.getData()[0]);
            return;
        }
        modifiable.setOpacity(interpolateValue(stateA.getData()[0], stateB.getData()[0], interpolationValue));
    }
}