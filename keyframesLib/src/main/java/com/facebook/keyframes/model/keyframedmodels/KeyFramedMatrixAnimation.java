...
public class KeyFramedMatrixAnimation extends KeyFramedObject<KFAnimationFrame, Matrix> {
    ...
    public static KeyFramedMatrixAnimation fromAnimation(KFAnimation animation) {
        ...
        return new KeyFramedMatrixAnimation(animation.getAnimationFrames(), animation.getTimingCurves(), animation.getPropertyType(), animation.getAnchor());
    }
    ...
    private KeyFramedMatrixAnimation(List<KFAnimationFrame> objects, float[][][] timingCurves, KFAnimation.PropertyType propertyType, float[] anchor) {
     ...
     */
    @Override
    protected void applyImpl(KFAnimationFrame stateA, KFAnimationFrame stateB, float interpolationValue, Matrix modifiable) {
        switch (mPropertyType) {
            ...
            case Y_POSITION:
                ...
                break;
            default:
                throw new UnsupportedOperationException("Cannot apply matrix transformation to " + mPropertyType);
        }
    }
     ...
     */
    private void applyRotation(KFAnimationFrame stateA, KFAnimationFrame stateB, float interpolationValue, Matrix modifiable) {
        if (stateB == null) {
            modifiable.postRotate(stateA.getData()[0], mAnchor != null ? mAnchor[0] : 0, mAnchor != null ? mAnchor[1] : 0);
            return;
        }
        float rotationStart = stateA.getData()[0];
        float rotationEnd = stateB.getData()[0];
        modifiable.postRotate(interpolateValue(rotationStart, rotationEnd, interpolationValue), mAnchor != null ? mAnchor[0] : 0, mAnchor != null ? mAnchor[1] : 0);
    }

    /**
     * This method applies a scale transformation to the matrix, interpolated between two states.
     */
    private void applyScale(KFAnimationFrame stateA, KFAnimationFrame stateB, float interpolationValue, Matrix modifiable) {
        if (stateB == null) {
            modifiable.postScale(stateA.getData()[0] / 100f, stateA.getData()[1] / 100f, mAnchor != null ? mAnchor[0] : 0, mAnchor != null ? mAnchor[1] : 0);
            return;
        }
        ...
        modifiable.postScale(interpolateValue(scaleStartX, scaleEndX, interpolationValue) / 100f, interpolateValue(scaleStartY, scaleEndY, interpolationValue) / 100f, mAnchor != null ? mAnchor[0] : 0, mAnchor != null ? mAnchor[1] : 0);
    }
     ...
     * states.
     */
    private void applyXPosition(KFAnimationFrame stateA, KFAnimationFrame stateB, float interpolationValue, Matrix modifiable) {
        if (stateB == null) {
        ...
        modifiable.postTranslate(interpolateValue(translationStartX, translationEndX, interpolationValue), 0);
    }
     ...
     * states.
     */
    private void applyYPosition(KFAnimationFrame stateA, KFAnimationFrame stateB, float interpolationValue, Matrix modifiable) {
        if (stateB == null) {
        ...
        modifiable.postTranslate(0, interpolateValue(translationStartY, translationEndY, interpolationValue));
    }
     ...
     */
    @Deprecated
    private void applyPosition(KFAnimationFrame stateA, KFAnimationFrame stateB, float interpolationValue, Matrix modifiable) {
        if (stateB == null) {
            return;
        }
        float translationStartX = stateA.getData()[0];
        ...
        modifiable.postTranslate(interpolateValue(translationStartX, translationEndX, interpolationValue) - mAnchor[0], interpolateValue(translationStartY, translationEndY, interpolationValue) - mAnchor[1]);
    }
}
