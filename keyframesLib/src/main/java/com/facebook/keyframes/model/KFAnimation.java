...
public class KFAnimation {
    public static final Comparator<KFAnimation> ANIMATION_PROPERTY_COMPARATOR = new Comparator<KFAnimation>() {
        @Override
        public int compare(KFAnimation lhs, KFAnimation rhs) {
    ...
    public KFAnimation(PropertyType propertyType, List<KFAnimationFrame> animationFrames, float[][][] timingCurves, float[] anchor) {
        mPropertyType = ArgCheckUtil.checkArg(propertyType, propertyType != null, PROPERTY_TYPE_JSON_FIELD);
        mAnimationFrames = ArgCheckUtil.checkArg(ListHelper.immutableOrEmpty(animationFrames), animationFrames != null && animationFrames.size() > 0, ANIMATION_FRAMES_JSON_FIELD);
        mTimingCurves = ArgCheckUtil.checkArg(timingCurves, ArgCheckUtil.checkTimingCurveObjectValidity(timingCurves, mAnimationFrames.size()), TIMING_CURVES_JSON_FIELD);
        mAnchor = ArgCheckUtil.checkArg(anchor, anchor == null || anchor.length == 2, ANCHOR_JSON_FIELD);
        if (mPropertyType.isMatrixBased()) {
            ...
            throw new IllegalArgumentException("Unknown property type for animation post processing: " + mPropertyType);
        }
    }
