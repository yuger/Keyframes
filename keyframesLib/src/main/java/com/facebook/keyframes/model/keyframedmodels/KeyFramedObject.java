...
public abstract class KeyFramedObject<T extends HasKeyFrame, M> {
    ...
    public void apply(float frameProgress, M modifiable) {
        if (mInterpolators.isEmpty() || frameProgress <= mFirstDescribedFrame) {
        ...
        for (interpolatorIndex = 0; interpolatorIndex < len; interpolatorIndex++) {
            if (mObjects.keyAt(interpolatorIndex) == frameProgress || (mObjects.keyAt(interpolatorIndex) < frameProgress && mObjects.keyAt(interpolatorIndex + 1) > frameProgress)) {
                thisFrame = mObjects.valueAt(interpolatorIndex);
                nextFrame = mObjects.valueAt(interpolatorIndex + 1);
                break;
            }
        }
        float progress = (frameProgress - thisFrame.getKeyFrame()) / (nextFrame.getKeyFrame() - thisFrame.getKeyFrame());
        applyImpl(thisFrame, nextFrame, mInterpolators.get(interpolatorIndex).getInterpolation(progress), modifiable);
    }
     ...
     */
    protected abstract void applyImpl(T stateA, T stateB, float interpolationValue, M modifiable);
