 ...
 * KFAnimation.
 */
public class KeyFramedStrokeWidth extends KeyFramedObject<KFAnimationFrame, KeyFramedStrokeWidth.StrokeWidth> {
    /**
    ...
    public KeyFramedStrokeWidth(List<KFAnimationFrame> objects, float[][][] timingCurves) {
     ...
     */
    @Override
    protected void applyImpl(KFAnimationFrame stateA, KFAnimationFrame stateB, float interpolationValue, StrokeWidth modifiable) {
        if (stateB == null) {
            modifiable.setStrokeWidth(stateA.getData()[0]);
            return;
        }
        modifiable.setStrokeWidth(interpolateValue(stateA.getData()[0], stateB.getData()[0], interpolationValue));
    }
}
