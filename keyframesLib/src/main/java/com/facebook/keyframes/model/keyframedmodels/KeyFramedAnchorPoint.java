...
public class KeyFramedAnchorPoint extends KeyFramedObject<KFAnimationFrame, Matrix> {
    ...
    private KeyFramedAnchorPoint(List<KFAnimationFrame> objects, float[][][] timingCurves) {
        super(objects, timingCurves);
    }

    @Override
    protected void applyImpl(KFAnimationFrame stateA, KFAnimationFrame stateB, float interpolationValue, Matrix matrix) {
        if (stateB == null) {
            matrix.postTranslate(-stateA.getData()[0], -stateA.getData()[1]);
            return;
        }
        matrix.postTranslate(-interpolateValue(stateA.getData()[0], stateB.getData()[0], interpolationValue), -interpolateValue(stateA.getData()[1], stateB.getData()[1], interpolationValue));
    }
}
