...
public class KFAnimationGroup {
    ...
    public KFAnimationGroup(int groupId, int parentGroup, List<KFAnimation> animations) {
        mGroupId = ArgCheckUtil.checkArg(groupId, groupId > 0, GROUP_ID_JSON_FIELD);
        mParentGroup = parentGroup;
        ListHelper.sort(animations, KFAnimation.ANIMATION_PROPERTY_COMPARATOR);
        mAnchorPoint = AnimationHelper.extractSpecialAnimationAnimationSet(animations, KFAnimation.PropertyType.ANCHOR_POINT);
        mAnimations = ArgCheckUtil.checkArg(ListHelper.immutableOrEmpty(animations), animations.size() > 0, ANIMATIONS_JSON_FIELD);
    }
