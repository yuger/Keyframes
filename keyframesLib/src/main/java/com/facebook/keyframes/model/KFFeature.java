...
public class KFFeature {
    ...
    public static class Builder {
        ...
        public KFFeature build() {
            return new KFFeature(name, fillColor, strokeColor, strokeWidth, fromFrame, toFrame, keyFrames, timingCurves, animationGroup, strokeLineCap, featureMask, featureAnimations, anchorPoint, effect, className);
        }
    }

    public KFFeature(String name, int fillColor, int strokeColor, float strokeWidth, float fromFrame, float toFrame, List<KFFeatureFrame> keyFrames, float[][][] timingCurves, int animationGroup, Paint.Cap strokeLineCap, KFFeature featureMask, List<KFAnimation> featureAnimations, float[] anchorPoint, KFFeatureEffect effect, String className) {
        mName = name;
        ...
        mTimingCurves = ArgCheckUtil.checkArg(timingCurves, ArgCheckUtil.checkTimingCurveObjectValidity(timingCurves, mKeyFrames.size()), TIMING_CURVES_JSON_FIELD);
        mAnimationGroup = animationGroup;
        mStrokeLineCap = strokeLineCap;
        mFeatureMask = featureMask;
        mStrokeWidthAnimation = AnimationHelper.extractSpecialAnimationAnimationSet(featureAnimations, KFAnimation.PropertyType.STROKE_WIDTH);
        mAnchorPoint = AnimationHelper.extractSpecialAnimationAnimationSet(featureAnimations, KFAnimation.PropertyType.ANCHOR_POINT);
        mOpacityAnimation = AnimationHelper.extractSpecialAnimationAnimationSet(featureAnimations, KFAnimation.PropertyType.OPACITY);
    ...
    public void setStrokeWidth(KeyFramedStrokeWidth.StrokeWidth strokeWidth, float frameProgress) {
        if (strokeWidth == null) {
            return;
        }
    ...
    public void setOpacity(KeyFramedOpacity.Opacity opacity, float frameProgress) {
