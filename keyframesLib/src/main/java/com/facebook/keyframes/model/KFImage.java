...
public class KFImage {
    ...
    private KFImage(int frameRate, int frameCount, List<KFFeature> features, List<KFAnimationGroup> animationGroups, float[] canvasSize, int key) {
        mFrameRate = ArgCheckUtil.checkArg(frameRate, frameRate > 0, FRAME_RATE_JSON_FIELD);
        mFrameCount = ArgCheckUtil.checkArg(frameCount, frameCount > 0, FRAME_COUNT_JSON_FIELD);
        mFeatures = ArgCheckUtil.checkArg(ListHelper.immutableOrEmpty(features), features.size() > 0, FEATURES_JSON_FIELD);
        animationGroups = AnimationGroupSort.sort(animationGroups);
        mAnimationGroups = ArgCheckUtil.checkArg(ListHelper.immutableOrEmpty(animationGroups), ArgCheckUtil.checkAnimationGroupIdUniqueness(animationGroups), ANIMATION_GROUPS_JSON_FIELD);
        mCanvasSize = ArgCheckUtil.checkArg(canvasSize, canvasSize.length == 2 && canvasSize[0] > 0 && canvasSize[1] > 0, CANVAS_SIZE_JSON_FIELD);
        mKey = key;
    }
    ...
    public void setAnimationMatrices(SparseArray<Matrix> matrixMap, float frameProgress) {
        Matrix matrix;
        for (int groupIndex = 0, groupsLen = mAnimationGroups.size(); groupIndex < groupsLen; groupIndex++) {
            ...
            for (int animationIndex = 0, animationsLen = group.getAnimations().size(); animationIndex < animationsLen; animationIndex++) {
