...
public abstract class VectorCommand {
        ...
        switch (cmd) {
            ...
            case M: {
                if (checkArguments(cmd, args)) {
                    ...
                    throw new IllegalArgumentException(String.format(Locale.US, "VectorCommand MoveTo requires two arguments, but got %s", args.toString()));
                }
            }
            case q:
            case Q: {
                if (checkArguments(cmd, args)) {
                    ...
                    throw new IllegalArgumentException(String.format(Locale.US, "VectorCommand QuadraticTo requires four arguments, but got %s", args.toString()));
                }
            }
            case c:
            case C: {
                if (checkArguments(cmd, args)) {
                    ...
                    throw new IllegalArgumentException(String.format(Locale.US, "VectorCommand CubicTo requires six arguments, but got %s", args.toString()));
                }
            }
            case l:
            case L: {
                if (checkArguments(cmd, args)) {
                    ...
                    throw new IllegalArgumentException(String.format(Locale.US, "VectorCommand LineTo requires two arguments, but got %s", args.toString()));
                }
            }
            default: {
                throw new IllegalArgumentException(String.format(Locale.US, "Unhandled vector command: %s", svgCommandString));
            }
        }
    }
    ...
    public static float[] convertUp(float[] startPoint, float[] fromArgs, float[] destArgs) {
        ...
        if (fromArgs.length == 2) {
            if (destArgs.length == 4) {
                ...
                throw new IllegalArgumentException(String.format("Unknown conversion from %d args to %d", fromArgs.length, destArgs.length));
            }
     ...
     */
    public void interpolate(VectorCommand toCommand, float progress, KFPath destPath) {
    ...
    public static class MoveToCommand extends VectorCommand {
        ...
        protected void applyInner(KFPath path, ArgFormat format, float[] args) {
            switch (format) {
                ...
                default: {
                    throw new IllegalArgumentException(String.format(Locale.US, "No such argument format %s", format));
                }
            }
        }
    ...
    public static class QuadraticToCommand extends VectorCommand {
        ...
        protected void applyInner(KFPath path, ArgFormat format, float[] args) {
            switch (format) {
                case RELATIVE: {
                    path.rQuadTo(args[0], args[1], args[2], args[3]);
                    break;
                }
                case ABSOLUTE: {
    ...
    public static class CubicToCommand extends VectorCommand {
        ...
        protected void applyInner(KFPath path, ArgFormat format, float[] args) {
            switch (format) {
                case RELATIVE: {
                    path.rCubicTo(args[0], args[1], args[2], args[3], args[4], args[5]);
                    break;
                }
                case ABSOLUTE: {
